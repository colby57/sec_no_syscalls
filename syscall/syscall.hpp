...
namespace syscall {
    inline std::mutex g_syscall_mutex;
    ...
    inline std::unordered_map<std::string, syscall_entry> g_syscall_map;
    inline void* g_syscall_region = nullptr;
    inline size_t g_region_size = 0;
    inline bool g_initialized = false;

    inline constexpr uint8_t syscall_shellcode[] = {
        0x51,                           // push rcx
        0x41, 0x5A,                     // pop r10
    ...
    inline constexpr size_t syscall_stub_size = sizeof (syscall_shellcode);

    namespace detail {
        inline uint32_t rva_to_offset (uint32_t rva, IMAGE_NT_HEADERS* nt_headers) {
    ...
    inline bool initialize () {
        ...
        if (detail::extract_syscalls ()) {
            g_initialized = detail::create_syscalls ();
        }
    ...
    inline std::optional<uint32_t> get_syscall_number (const std::string& function_name) {
        std::lock_guard<std::mutex> lock (g_syscall_mutex);
    ...
    template<typename T>
    inline T get_syscall_func (const std::string& function_name) {
        std::lock_guard<std::mutex> lock (g_syscall_mutex);
